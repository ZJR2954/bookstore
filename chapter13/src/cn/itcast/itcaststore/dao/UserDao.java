...
public class UserDao {
	...
	public void changeUser(User user,User nuser) throws SQLException {
		// TODO 自动生成的方法存根		
		List<Object> params=new ArrayList<Object>();
		String sql="update user set gender=? ";
		params.add(nuser.getGender());
		if(nuser.getPassword()!=null && nuser.getPassword().trim().length()>0) {
			sql+=" ,password=?";
			params.add(nuser.getPassword());
		}
		sql+=" ,telephone=? where id=?";	
		params.add(nuser.getTelephone());
